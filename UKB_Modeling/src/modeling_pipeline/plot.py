    ...
    *,  # Enforce keyword-only arguments
    ...
    if True:
        ...
        if save_fig:
            ...
            _path=os.path.join(fig_path,'auc_over_time')
            os.makedirs(_path, exist_ok=True)
            adjusted_path = os.path.join(_path, f"{name}_time_dep_AUC_{metric_suffix}_{target}.{export_format}")
...
def shap_analysis(
    ...
    if save_fig:
        export_file = os.path.join(export_path, f"SHAP_{self.user_input.col_subset}_{self.user_input.row_subset}_{max_display}.{export_format}")
