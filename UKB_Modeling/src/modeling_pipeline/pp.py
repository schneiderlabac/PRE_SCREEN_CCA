...
def load_data(self, subset, test_cohort = "val",use_raw=False,dataframes={}):
    ...
    filepaths = {
        "inner": {
            ...
            "y": os.path.join(data_path, f"y_inner_{subset}")
        },
        "outer": {
            "X": os.path.join(data_path, f"X_outer_{subset}{use_raw__}"),
            "y": os.path.join(data_path, f"y_outer_{subset}")
        }
    }
    ...
    if '____' in X_ohe.columns[0]: # only run for multi cancer because other option is shit
        ...
        for el in X_ohe.columns.tolist():
            if el.startswith(prefix_ohe):
                ...
                source = columngroups_X.get('__'.join(el.split("__")[1:]))
...
class pp_user_input:
    def __init__(self, project_vars: dict):
        ...
        self.export_date_data = project_vars["export_date_data"]
        ...
        for key, value in self.hyperparameters.items():
            ...
            print(f"{key}: Detected n_jobs_indiv = {params_pipeline.get('n_jobs_indiv')}")
        ##################################################################################################################################
        ...
        elif self.col_subset in ["Model_E", "Model_Metabolomics", "Model_Evec", "Model_Evectab", "Model_Efemale", "Model_Enotscreened"]:  # type: ignore
        ...
        elif self.row_subset == "unscreened":  # type: ignore
    ...
    @property
    def path(self):
        return userpath(os.environ.get("USER", os.environ.get("USERNAME")), project=self.project)
