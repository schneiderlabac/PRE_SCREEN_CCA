...
def plot_precision_recall_curves(
    ...
    def create_clean_label(key):
        ...
        if 'Model' in label_parts:
            ...
            parts = [p.strip() for p in (top_part, model_type) if p and p.strip()]
            clean_label = ' - '.join(parts)
    ...
    for key in keys_ordered:
        try:
            ...
            clean_label = create_clean_label(key)
            ax.plot(recall, precision, lw=lw, linestyle=linestyle,
                    color=colors[key], label=f'{clean_label} ({pr_auc:.3f})', alpha=1.0)
        ...
        for ext_key in external_keys:
            try:
                if ext_key in external_prc_df.index:
                    ...
                    # Plot external curve
                    ax.plot(recall_ext, precision_ext, lw=lw, linestyle=linestyle_ext,
                            color=color_ext, label=f'{clean_label} ({pr_auc_ext:.3f})', alpha=1.0)
    ...
    if plot_count > 0:
        ...
        if title:
            ...
            ax.set_title(display, fontsize=font_size + 4, pad=10)
        ...
        if plot_legend:
            ...
            if handles:  # Only add legend if we have items to display
                legend = ax.legend(handles=handles, loc='upper right', frameon=True,
                                 fontsize=font_size-2)  # Match first function's font size reduction
