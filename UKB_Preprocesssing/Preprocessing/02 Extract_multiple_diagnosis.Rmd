...
par_subset <- par_subset[!par_subset %in% c("Blood_Parameters","Lifestyle")] #Remove the Blood parameters as diagnosis label, only hard-coded diagnosis are of interest here
...
######################################
NOD_cutoff=1 # newly onset diabetes, symmetrically around the baseline ass. visit
######################################
...
icd_dm <- icd_dm[order(icd_dm$epistart,decreasing = TRUE), ]
icd_dm<- as.data.frame(icd_dm[!duplicated(icd_dm$eid),]) # #Step 2: Duplikate entfernen:
icd_dm$DM <- 1
icd_dm$NOD <- ifelse(((icd_dm$difftime > -1 ) & (icd_dm$difftime < NOD_cutoff)), 1, 0)
icd_dm<-select(icd_dm, c(eid, DM, NOD))
```
...
print(paste("Nr of patients without any NHS entries in the specified timeframe:", nrow(df_icd) - sum(complete.cases(df_icd))))
...
setwd(project_path)
load(paste0(project_path,"/data/dataframes/df_blood.RData"))
load(paste0(project_path,"/data/dataframes/df_covariates.RData"))
...
if (project_key == "pancaa") {
  df_blood_risk <- df_blood %>%
  select(c("eid", "Aspartate aminotransferase", "Alanine aminotransferase", "Gamma glutamyltransferase", "Alkaline phosphatase" )) %>%
  inner_join(select(df_covariates, eid, SEX,AGE,"Pack years"), by = "eid") %>%
  mutate(
    Elevated_AST = if_else(SEX == "Female" & `Aspartate aminotransferase` > 35 | SEX == "Male" & `Aspartate aminotransferase` > 50, 1, 0),
    Elevated_ALT = if_else(SEX == "Female" & `Alanine aminotransferase` > 35 | SEX == "Male" & `Alanine aminotransferase` > 50, 1, 0),
    Elevated_GGT = if_else(SEX == "Female" & `Gamma glutamyltransferase` > 40 | SEX == "Male" & `Gamma glutamyltransferase` > 60, 1, 0),
    Elevated_AP = if_else(SEX == "Female" & `Alkaline phosphatase` > 105 | SEX == "Male" & `Alkaline phosphatase` > 130, 1, 0),
    Advanced_AGE = if_else(AGE > 65,1,0),
    Heavy_Smoking = if_else(`Pack years` > 30 ,1,0)
  ) %>%
  mutate(Elevated_Liver_Enzymes = if_else(Elevated_AST == 1 | Elevated_ALT == 1 | Elevated_GGT == 1, 1, 0)) %>%
  select(-c("Aspartate aminotransferase", "Alanine aminotransferase", "Gamma glutamyltransferase", "SEX", "Alkaline phosphatase",AGE,"Pack years"))
...
write.csv(df_diagnosis, file=paste(project_path, "/data/dataframes/df_diagnosis.csv", sep=''))
...
sum(df_y$status_cancerreg)

df_mergeeed <- inner_join(df_y, df_par, by="eid")
sum(df_mergeeed$status_cancerreg)
...
sum_pos <- left_join(sum_diagnosis, sum_positives, by="Diagnosis") %>%
  ...
  filter(Diagnosis != "Hepatobiliary cancer") %>%
  arrange(desc(Percentage.y)) %>%
  mutate(Legend = LETTERS[1:57])




comparison_plot_diags(sum_pos, base_size=20)
