...
if (DOI == "CCA") {
  vec_risk_constellation <- par_index$Diagnosis[par_index$Group %in% par_subset]
} else if (DOI == "HCC") {
...
#a) df_eid 
setwd(sharepoint_ukb)
df_withdrawals <- read.delim("raw/withdrawals.csv")
...
```{r}
#Import metabolomics (obtained from EDTA plasma from baseline recruitment)
...
#write.csv(df_metabolomics, file="C:/Users/felix/Patient_tables/UKB_Patients_metabolomics.csv", row.names=FALSE)    
...
nrow(df_metabolomics) #correlates to number of participants still included
...
merge_dataframes <- function(include_metabolomics = FALSE, filter_par = FALSE, normalize = FALSE) {
        ...
        dfs_to_merge <- list(df_eid, df_covariates, df_diagnosis, df_blood, df_snp)
        ...
        if (normalize==TRUE) {
          df_merged <- normalize_data(df_merged, df_cap= df_cap_covariates, cap=TRUE)
...
normalize_data <- function(df, df_cap = NULL, cap = FALSE) {
  # Check if capping is requested but df_cap is missing
  if (cap && is.null(df_cap)) {
    stop("df_cap must be provided when cap = TRUE")
  }
  ...
  if (cap) {
    # Apply capping and normalization
    df <- cap_and_normalize(df, df_cap)
    
    # Apply minmax normalization to remaining columns
    remaining_cols <- setdiff(vec_all, df_cap$column)
  } else {
    # If not capping, apply minmax to all columns in vec_all
    remaining_cols <- vec_all
  }
  
  # Apply minmax normalization to remaining or all columns
  for (i in remaining_cols) {
    if (i %in% names(df) && is.numeric(df[[i]])) {
      df[[i]] <- minmax(df[[i]])
    }
  }
...
```{r}
#Remove Blood Risk vector from modeling, only used for PAR Subsetting
#df_diagnosis <- df_diagnosis %>% select(-all_of(vec_blood_risk))
...
if (export_normalized) {
  ...
  process_and_save(df_x_par_met, "par_cca")
}
```
...
#Optional load in of already created dataframes, so you do not need to run preprocess again
setwd(project_path)
df_x_all_inner <- read.csv("data/19_04_2025/X_inner_basic_all_cca_raw.csv")
df_x_all_outer <- read.csv("data/19_04_2025/X_outer_basic_all_cca_raw.csv")
df_x_all_raw <- bind_rows(df_x_all_inner, df_x_all_outer)
sum(df_x_all_raw$Hepatobiliary.cancer)

setwd(project_path)
load("data/dataframes/df_y.RData") 
df_y_inner <- read.csv("data/19_04_2025/y_inner_basic_all_cca_raw.csv")
df_y_outer <- read.csv("data/19_04_2025/y_outer_basic_all_cca_raw.csv")
df_y_all <- bind_rows(df_y_inner, df_y_outer)
df_merged  <- merge(df_x_all_raw, df_y , by = "eid")
sum(df_y_outer$status_cancerreg == 1)
```

```{r}
setwd(project_path)
df_columngroups <- read.csv("data/19_04_2025/columngroups.csv")
...
table1_order <- c(
  ...
  "MultipleDeprivationIndex", "Bloodpressure_sys", "Medication", 
  "Family_diabetes", "Smoking status", "Ever smoked", "Pack years", "Alk_g_d" # Continue as necessary BMI_cat
)
...
continuous_columns_present <- continuous_columns[continuous_columns %in% colnames(df_tbl_1)]
  type_list <- if (length(continuous_columns_present) > 0) {
    setNames(rep("continuous", length(continuous_columns_present)), continuous_columns_present)
  } else {
    NULL
  }
```
...
colnames(df_x_all_raw) <- gsub("\\.", " ", colnames(df_x_all_raw))
...
create_table(df_tbl_1, "Table 1_cca", export_RDS=TRUE, head_only=head_only, remove_SEX=FALSE,  enforced_order=table1_order)


df_tbl_1b <- df_all %>%
  select(!any_of(c(blood_list, metabolomics_list, icd_list, "eid", "Date of assessment", "UKB assessment centre", snp_list)))

create_table(df_tbl_1b, "Table 1b_cca", export_RDS=TRUE, head_only=head_only, remove_SEX=FALSE,  enforced_order=table1b_order)
```
...
create_table(df_tbl_1, "Table 1_par_ecca", export_RDS=TRUE, head_only=head_only, remove_SEX=FALSE,  enforced_order=table1_order)
```
...
create_table(df_tbl_blood, "Table Blood_cca", export_RDS=TRUE, head_only=head_only)

create_table(df_tbl_icd, "Table ICD_cca", export_RDS=TRUE, head_only=head_only, create_binary_table = TRUE)

create_table(df_tbl_metabolomics, "Table Metabolomics_cca", export_RDS=FALSE, head_only=head_only)

create_table(df_tbl_snp, "Table SNP_cca", export_RDS=TRUE, head_only=head_only)
...
split_create_merge_tables(df_tbl_1, table_name="Table1_cca", feature="SEX", enforced_order=table1_order, remove_SEX=TRUE, export_RDS=TRUE)

Table_1_stratified <- import_merge_tables(table_name= "Table1_cca", feature="SEX", levels = c("Female", "Male"))
...
split_create_merge_tables(df_tbl_blood, table_name= "Table_Blood_cca", feature="SEX", enforced_order=FALSE, remove_SEX=TRUE, export_RDS=TRUE)

Table_Blood_stratified <- import_merge_tables(table_name= "Table_Blood_cca", feature="SEX", levels = c("Female", "Male"))

# Stratified "ICD Table"
split_create_merge_tables(df_tbl_icd, table_name= "Table_ICD_cca", feature="SEX", enforced_order=FALSE, remove_SEX=TRUE, export_RDS=TRUE, create_binary_table = TRUE)

Table_ICD_stratified <- import_merge_tables(table_name= "Table_ICD_cca", feature="SEX", levels = c("Female", "Male"))

# Stratified "Metabolomics Table"
split_create_merge_tables(df_tbl_metabolomics, table_name= "Table_metabolomics_cca", feature="SEX", enforced_order=FALSE, remove_SEX=TRUE, export_RDS=TRUE)

Table_Metabolomics_stratified <- import_merge_tables(table_name= "Table_Metabolomics_cca", feature="SEX", levels = c("Female", "Male"))
...
# Table 1 for PMBB dataset
```{r}
setwd(project_path)
df_pmbb_all <- read.csv("data/df_pmbb_all.csv")
colnames(df_pmbb_all)[colnames(df_pmbb_all) == "status_cancerreg"] <- "status"
...
table_pmbb_order <- c(
  "AGE", "Sex", "Ethnicity","Pack.years","Alkaline.phosphatase", "Gamma.glutamyltransferase", "Triglycerides", "Glucose", "Obstruction.of.bile.duct", "Complications.of.cirrhosis", "Cholangitis", "Arterial.hypertension"  # Continue as necessary BMI_cat
)
...
df_pmbb_all$status <- ifelse(df_pmbb_all$status == 0, paste("No ", DOI), 
                                  ifelse(df_pmbb_all$status == 1, DOI, df_pmbb_all$status))


df_tbl_1 <- df_pmbb_all %>%
  select(!any_of(c( "PMBB_ID")))

create_table(df_tbl_1, "Table 1_cca_pmbb", export_RDS=TRUE, head_only=head_only, remove_SEX=FALSE,  enforced_order=table_pmbb_order)
